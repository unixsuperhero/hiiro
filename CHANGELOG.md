...
### Added
- `h branch save` now stores the full commit SHA of the branch's HEAD at save time



- `h sparse` — manage `~/.config/hiiro/sparse_groups.yml` via subcommands: `ls [group]`, `add <group> <path> [path...]`, `rm <group> [path...]`, `config`
...
### Fixed
- fix(tasks): add help text exit for sparse-checkout command
- fix(notify): disable window-unlinked hook
- fix(queue): skip markdown headers when auto-generating prompt names from content
- fix(pr): include tags in oneline PR display format

## [0.1.265] - 2026-03-19

### Added
- `h queue add -T` interactive session picker for non-task tmux sessions
- `h pane home` with add/rm/ls/switch subcommands

### Fixed
- fix(pane): use session path as fallback when creating new window without directory
